        ...
        <div id="deleteModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
            <div class="relative p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mt-3">¿Eliminar profesional?</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500">¿Estás seguro de que deseas eliminar este Profesional? Esta acción no se puede deshacer.</p>
                    </div>
            ...
            <div class="px-8 py-6 border-b border-gray-100">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div class="flex items-center mb-4 md:mb-0">
                        <i class="ri-user-star-line text-2xl text-gray-600 mr-3"></i>
                        <h1 class="text-2xl font-bold text-gray-800">Profesionales</h1>
                    </div>
                    <a href="{{ route('profesionales.create') }}" class="flex items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-md transition duration-200">
                        <i class="ri-add-line mr-2"></i> Crear Profesional
                    </a>
                </div>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Profesional</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Profesión</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Especialidad</th>
                        ...
                        <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex justify-center">
                                    <img src="{{ asset('storage/' . $profesional->foto) }}" alt="Foto de {{ $profesional->primer_nombre }}" class="h-12 w-12 rounded-full object-cover">
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">
                                    ...
                                    {{ $profesional->segundo_nombre ? $profesional->segundo_nombre . ' ' : '' }}
                                    {{ $profesional->primer_apellido }} 
                                    {{ $profesional->segundo_apellido ? $profesional->segundo_apellido : '' }}
                                </div>
                            </td>
                            
                            <!-- Profesión -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600">{{ $profesional->profesion }}</div>
                            </td>
                            
                            <!-- Especialidad -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600">{{ $profesional->especialidad }}</div>
                            </td>
                            
                            <!-- Estatus -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $profesional->estatus == 'Aprobado' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $profesional->estatus }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex justify-center space-x-3">
                                    <!-- Ver -->
                                    <a href="{{ route('profesionales.show', $profesional->id) }}" class="text-blue-500 hover:text-blue-700 transition-colors duration-200" title="Ver detalles">
                                        <i class="ri-eye-line text-lg"></i>
                                    </a>
                                    
                                    <!-- Editar -->
                                    <a href="{{ route('profesionales.edit', $profesional->id) }}" class="text-yellow-500 hover:text-yellow-700 transition-colors duration-200" title="Editar">
                                        <i class="ri-pencil-line text-lg"></i>
                                    </a>
                                
                                    <!-- Eliminar -->
                                    <button type="button" onclick="openModal('{{ route('profesionales.destroy', $profesional->id) }}')" class="text-red-500 hover:text-red-700 transition-colors duration-200" title="Eliminar">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center">
                    <div class="mb-4 md:mb-0">
                        <p id="resultCount" class="text-sm text-gray-500">
                            ...
                            a
                            <span class="font-medium text-gray-700">{{ min(10, count($profesionales)) }}</span>
                            de
                            <span class="font-medium text-gray-700">{{ count($profesionales) }}</span>
                            resultados
                        </p>
                    </div>
