...
<div class="card-header flex flex-col sm:flex-row justify-between items-start sm:items-center py-5 px-8 bg-gradient-to-r from-blue-50 to-blue-100 border-b border-gray-200">
    ...
    <!-- Botón Regresar con separación a la derecha -->
...
<script>
    ...
    function autofillForm() {
        ...
        // Datos de productos con descripciones y precios asociados
        const products = [
            { name: 'Laptop Gamer', description: 'Potente dispositivo con las mejores características del mercado.', price: 499.99 },
            { name: 'Smartphone Pro', description: 'Última tecnología para un rendimiento superior.', price: 999.99 },
            { name: 'Auriculares Inalámbricos', description: 'Diseño ergonómico y sonido envolvente.', price: 149.99 },
            { name: 'Cámara 4K', description: 'Captura momentos con calidad impresionante.', price: 799.99 },
            { name: 'Reloj Inteligente', description: 'Estilo y funcionalidad en tu muñeca.', price: 199.99 },
            { name: 'Monitor Curvo', description: 'Pantalla curva para una experiencia inmersiva.', price: 299.99 }
        ];

        // Selección aleatoria de un producto
        const randomProduct = products[Math.floor(Math.random() * products.length)];

        // Rellenar los campos con datos coherentes
        setValue('input[name="nombre"]', randomProduct.name);
        setValue('input[name="precio"]', randomProduct.price);
        setValue('textarea[name="descripcion"]', randomProduct.description);
        ...
        Swal.fire({
            ...
            html: `Se han completado los campos con datos de prueba:<br>
                   <strong>Nombre:</strong> ${randomProduct.name}<br>
                   <strong>Precio:</strong> $${randomProduct.price.toFixed(2)}`,
            timer: 3000,
