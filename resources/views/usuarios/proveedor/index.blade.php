...
<div id="userModal" class="fixed inset-0 bg-gray-400/30 backdrop-blur-sm hidden flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl overflow-hidden w-full max-w-2xl shadow-lg">
        ...
        <!-- Pie del modal con botones responsivos -->
<div class="bg-gray-50 px-5 py-3 border-t border-gray-200">
    <div class="flex flex-col sm:flex-row sm:flex-wrap sm:justify-end gap-2">
        <button id="btnVerInfo" onclick="loadUserInfo()"
            class="bg-blue-400 hover:bg-blue-500 text-white px-4 py-2 rounded flex items-center justify-center transition-colors duration-200 text-sm w-full sm:w-auto">
            <i class="ri-eye-line mr-1 text-sm"></i> Ver Información
        </button>
        <a id="editUserBtn" href="#"
            class="bg-amber-400 hover:bg-amber-500 text-white px-4 py-2 rounded flex items-center justify-center transition-colors duration-200 text-sm w-full sm:w-auto">
            <i class="ri-edit-line mr-1 text-sm"></i> Editar
        </a>
        <form id="deleteUserForm" method="POST" class="w-full sm:w-auto">
            @csrf
            @method('DELETE')
            <button type="submit"
                class="bg-rose-400 hover:bg-rose-500 text-white px-4 py-2 rounded flex items-center justify-center transition-colors duration-200 text-sm w-full sm:w-auto"
                onclick="return confirm('¿Estás seguro de eliminar este usuario?')">
                <i class="ri-delete-bin-line mr-1 text-sm"></i> Eliminar
            </button>
        </form>
        <button onclick="closeUserModal()"
            class="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded flex items-center justify-center transition-colors duration-200 text-sm w-full sm:w-auto">
